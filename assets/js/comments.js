...
(function () {
  try {
    ...
    if (!s) {
      console.warn(
        "[comments] #comment-section NOT FOUND -> script will exit early"
      );
    } else {
      console.info("[comments] #comment-section FOUND", {
        postId: s.dataset.postId,
        listUrl: s.dataset.listUrl,
        itemUrl: s.dataset.itemUrl,
        aroundUrl: s.dataset.aroundUrl,
      });
    }
  } catch (e) {
...
(function () {
    ...
    try {
      ...
      if (typeof data.nextCursor === "string" && data.nextCursor) {
        ...
        const serverItems = Array.from(
          list.querySelectorAll('.comment-item[data-origin="server"]')
        );
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!hasHtml && newId && itemUrlBase) {
        try {
          const r = await fetch(`${itemUrlBase}/${newId}`, {
            credentials: "same-origin",
          });
          if (r.ok) {
          ...
          console.debug("[create.fallback] item fetch failed or empty html", {
            newId,
          });
        } catch (err) {
  ...
  function insertByPathToTopList(newEl) {
    ...
    const siblings = Array.from(
      list.querySelectorAll(":scope > .comment-item")
    ); // 직계만
    let placed = false;
  ...
  function firstShownPath() {
    const first =
      list && list.querySelector(":scope > .comment-item:first-child");
    return first ? first.dataset.path || "" : "";
  }
  function lastShownPath() {
    const last =
      list && list.querySelector(":scope > .comment-item:last-child");
    ...
    window.__toast(`새 댓글이 달렸어요: ${snippet}`, meta, async () => {
      try {
        ...
        if (!shouldInsertIntoCurrentWindow(path)) {
          ...
          const r2 = await fetch(`${aroundUrlBase}/${postId}?${qs}`, {
            credentials: "same-origin",
          });
          if (r2.ok) {
  ...
  async function ensureItemPresent({ path = "", cid = 0 }) {
    ...
    try {
      const r = await fetch(`${itemUrlBase}/${cid}`, {
        credentials: "same-origin",
      });
      if (!r.ok) return;
      const j = await r.json();
  ...
  function focusByPathOrId(path, cid) {
    ...
    let target = document.querySelector(
      `.comment-item[data-path="${safe(path)}"]`
    );
  ...
  async function ensureThreadVisible(childFullPath) {
    ...
    try {
      ...
      const r = await fetch(`${aroundUrlBase}/${postId}?${qs}`, {
        credentials: "same-origin",
      });
      if (!r.ok) return;
  ...
  (function () {
    ...
    es.addEventListener("comment", async (e) => {
      try {
        ...
        if (cid && itemUrlBase) {
          const res = await fetch(`${itemUrlBase}/${cid}`, {
            credentials: "same-origin",
          });
          if (res.ok) {
            ...
            if (
              ...
              if (node) {
                ...
                console.debug(
                  ...
                  winNextCursor,
                  "=> insert?",
                  shouldInsertIntoCurrentWindow(centerPath)
                );
  ...
  function enterWindowMode(html, cursors, { behavior = "replace" } = {}) {
    ...
    // 화면 낮을 때 아래 보충
    queueMicrotask(() => {
      if (winHasNext) loadDownInWindow();
    });
  }
  ...
  function initBotIO() {
    botIO = new IntersectionObserver(
      ([e]) => {
  ...
  async function loadUpInWindow() {
    ...
    try {
      ...
      const qs = new URLSearchParams({
        centerPath: winPrevCursor,
        before: 200,
        after: 0,
      });
      const url = `${listUrl.replace(
        /list_json.*/,
        ""
      )}around_json/${postId}?${qs}`;
  ...
  function trimListFromBottomIfTooMany(max = 400) {
    ...
    for (
      ...
      i >= 0 && i >= items.length - removeCount;
      i--
    ) {
      items[i].remove();
    }
